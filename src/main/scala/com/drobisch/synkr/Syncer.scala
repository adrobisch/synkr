...
case class VersionedFile(container: Option[String], name: String, version: Long, content: InputStream)
...
class Syncer(configuration: SyncerConfiguration,
  ...
  def sync: Unit = {
    configuration.fileSyncs.map(config => for {
      ...
      case 0 => log.debug(s"equal: $remoteFile, $localFile")
      case c if c < 0 => updateRemote(localFile, remoteFile)
      case c if c > 0 => updateLocal(localFile, remoteFile)
    })
  }
  ...
  def updateLocal(localFile: VersionedFile, remoteFile: VersionedFile) = {
    ...
    localFileBackend.putFile(localFile.container, localFile.name, remoteFile.content, Some(remoteFile.version))
  }

  def updateRemote(localFile: VersionedFile, remoteFile: VersionedFile) = {
    log.info("updating remote:")
    backup(remoteFile, "remote")
    remoteFileBackend.putFile(remoteFile.container, remoteFile.name, localFile.content, Some(localFile.version))
  }

  def backup(file: VersionedFile, infix: String): Option[String] = {
    localFileBackend.putFile(configuration.backupContainer, file.name + s".$infix." + file.version, file.content, Some(file.version))
  }
}
